...
export default {
    ...
    async isStudent(req,res, next){
        ...
        try{
            ...
            else if (accountType ===  'Admin' && (req.route.path =='/chosen_projects/:id' || req.route.path =='/template' || req.route.path == '/approved_project/:id') && req.route.methods.get == true)  return next() 
            else  {
                res.status(401).send()
                return false
            }
